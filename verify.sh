...
# Find the actual session directory (nested UUIDs)
SESSION_DIR=$(find "$SESSION_BASE" -name "local_*.json" -maxdepth 3 -print -quit 2>/dev/null)
...
" 2>/dev/null || echo "unknown")
    ...
    # =====================================================
    stale_paths_json=$(grep -c "/Users/[^${CURRENT_USER}]" "$json_file" 2>/dev/null || echo "0")
    # More precise check: look for /Users/<anything-except-current-user>/
    stale_user=""
    ...
    if [ -n "$stale_user" ]; then
        ...
        if [ "$FIX_PATHS" = "true" ]; then
            ...
            # Fix inside session directory too
